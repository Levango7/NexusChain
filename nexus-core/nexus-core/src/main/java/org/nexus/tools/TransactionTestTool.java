...
import org.nexus.util.AsynchronousHttpClient;
import org.nexus.util.monad.Monad;
...
/**
 * 事务发送工具
 * 使用方法 .\gradlew runTransactionTestTool -PappArgs="-c F:\java-nexuscore\transaction-test-example.jsonc"
 * 命令行参数会覆盖配置文件里的参数配置
 * -c 配置文件路径，尽量用绝对路径
 * -k 十六进制编码的私钥
 * -h --host 主机名或者 ip
...
public class TransactionTestTool {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (TransactionInfo info : testConfig.transactions) {
            ...
            BigDecimal amount = info.amount.multiply(new BigDecimal(EconomicModel.NEX));
            ...
            tx.gasPrice = (long) Math.ceil(
                    0.002 * EconomicModel.NEX / Transaction.GAS_TABLE[tx.type]
            );
        ...
        if (testConfig.protocol != null && testConfig.protocol.equals("grpc")) {
            final Peer self = Peer.newPeer("nexus://localhost:9585");
    ...
    private static void sendTransactionsByGRPC(List<Transaction> transactions, Peer self, TestConfig testConfig) throws Exception {
        NexusChainOuterClass.Transactions.Builder builder = NexusChainOuterClass.Transactions.newBuilder();
        ...
        List<NexusChainOuterClass.Transactions> transactionsList = Util.split(builder.build());
