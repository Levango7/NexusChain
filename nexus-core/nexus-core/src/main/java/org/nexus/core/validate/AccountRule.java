...
import org.nexus.command.TransactionCheck;
import org.nexus.consensus.pow.PackageMiner;
import org.nexus.core.Block;
import org.nexus.core.WhitelistTransaction;
...
import org.nexus.db.AccountState;
...
import static org.nexus.core.account.Transaction.Type.EXIT_MORTGAGE;
import static org.nexus.core.account.Transaction.Type.EXIT_VOTE;
...
public class AccountRule implements BlockRule {
    ...
    @Autowired
    NexusChainBlockChain nexusBlockChain;
    ...
    public Result validateBlock(Block block) {
        ...
        if (block.nHeight > 0) {
            for (Transaction tx : block.body) {
                ...
                // exchange 节点(validateIncubator=false)跳过孵化器交易撤回检查，
                // 但仍执行后续基本格式校验和账户更新，避免跳过全部验证
                ...
                if (tx.type != Transaction.Type.COINBASE.ordinal()) {
                    ...
                    if (tx.type == Transaction.Type.TRANSFER.ordinal()
                        ...
                        if (map.containsKey(tohash)) {
                            ...
                            toaccountState = new AccountState(toaccount, new HashMap<>(), new HashMap<>());
                        }
                        ...
                        if (cancelaccountList.containsKey("fromaccount")) {
                            ...
                            map.put(Hex.encodeHexString(tx.to), tovoteaccountState);
                        }
