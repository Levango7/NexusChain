...
public class CheckPointRule implements BlockRule {
    ...
    @Autowired
    private NexusChainBlockChain nexusBlockChain;

    public CheckPointRule(@Value("${nexus.open-check-point}") boolean openCheckPoint) {
    ...
    public Result validateDBCheckPoint() {
        ...
        for (long h : heights) {
            Block b = nexusBlockChain.getCanonicalBlock(h);
