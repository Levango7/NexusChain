...
//            }
            ...
            if (account != null) {
                ...
                if (type == 0x01 || type == 0x09) {
                    ...
                    //求余
                    long remainder = tranbalance % EconomicModel.NEX;
                    if (remainder != 0) {
                        apiResult.setCode(5000);
    ...
    private APIResult CheckHatch(long amount, byte[] payload, byte[] topubkeyhash) {
        ...
        try {
            ...
            if (amount < 30000000000L) {
                apiResult.setCode(5000);
                apiResult.setMessage("Minimum incubation amount: 300nexus");
                return apiResult;
            }
    ...
    private APIResult CheckInterAndShare(int type, long amount, byte[] payload, Incubator incubator, byte[] topubkeyhash) {
        ...
        try {
            ...
            Transaction transaction = nexusBlockChain.getTransaction(payload);
            if (transaction == null) {
                apiResult.setCode(5000);
