    ...
    public void configureTasks(ScheduledTaskRegistrar taskRegistrar) {
        taskRegistrar.addTriggerTask(() -> {
            ...
            for (TransPool transPool : list) {
                ...
                //db
                Transaction transaction = nexusBlockChain.getTransaction(t.getHash());
                if (transaction != null) {
                    maps.put(new String(Hex.encodeHexString(frompubhash)), adoptTransPool.getKeyTrans(t));
                }
            }
            ...
            for (TransPool transPool : pendinglist) {
                ...
                if (transPool.getState() == 1) {
                    ...
                    long nowheight = nexusBlockChain.getCurrentTotalWeight();
                    if (nowheight >= height) {
                        Block b = nexusBlockChain.getCanonicalBlock(height);
                        if (b != null) {
                            boolean state = true;
