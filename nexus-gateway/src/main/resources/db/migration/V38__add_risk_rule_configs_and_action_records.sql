...
CREATE TABLE risk_rule_configs (
    ...
    config_json   TEXT         COMMENT '扩展配置（JSON 格式，存储规则特定参数）',
...
CREATE TABLE risk_action_records (
    ...
    executed_at     TIMESTAMP(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) COMMENT '处置执行时间',
    completed_at    TIMESTAMP(6) NULL DEFAULT NULL COMMENT '处置完成时间',
    created_at      TIMESTAMP(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) COMMENT '创建时间',
    updated_at      TIMESTAMP(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6) COMMENT '更新时间',
    version         BIGINT       NOT NULL DEFAULT 0 COMMENT '乐观锁版本号',
    PRIMARY KEY (id),
    INDEX idx_rar_merchant_id (merchant_id),
    INDEX idx_rar_order_id (order_id),
