 ...
 *       submitted on-chain via {@link OnChainExecutionClient}.</li>
...
public class ColdWalletMultiSigService {
     ...
     * @param executionClient on-chain execution client
     */
    @Autowired
    public ColdWalletMultiSigService(MpcSigner signer,
        ...
        log.info("ColdWalletMultiSigService initialised");
    }
    ...
    public String aggregateAndBroadcast(String sessionId) {
        ...
        WalletTransactionRequest execReq = new WalletTransactionRequest(
                WalletTransactionRequest.Type.WITHDRAWAL,
                ctx.fromAddress,
                ctx.toAddress,
                ctx.amount,
                ctx.asset,
                "mpc-sig:" + signatureHex,
                ctx.requestId);

        WalletTransactionResult result = executionClient.execute(execReq);
        if (result == null || !result.isSuccess()) {
            String error = result == null ? "execution client returned null" : result.getError();
            session.markFailed(
                    MpcProtocolException.Reason.ILLEGAL_STATE,
                    "on-chain execution failed: " + error,
                    null);
        ...
        ctx.chainTxHash = result.getTxHash();
