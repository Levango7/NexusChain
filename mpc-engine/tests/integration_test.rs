...
// 集成测试使用 .expect(&format!(...)) 和 for i in 0..N 索引模式，
// 触发 clippy::expect_fun_call 和 clippy::needless_range_loop 风格 lint。
// 测试代码可读性优先，抑制这两个风格 lint。
#![allow(clippy::expect_fun_call, clippy::needless_range_loop)]
