...
> **文档版本**：v1.1（2026-09-17 加注）
> **生成日期**：2026-08-10（基线 v2.0.0-rc1）
>
> **历史快照说明**：本文件主体是 2026-08-10 的时点快照，其中多数条目
> 已在此后版本解除（如 §2.2 MPC E2E、§2.6 两个历史排除）。
> **当前状态以 CI 实跑为准**：2026-09-17 实测全仓 `@Disabled` 仅 1 处
> （`BlocksCacheTest`，见 §2.3，仍有效）、`Assumptions.assumeTrue` 29 处
> （环境驱动条件跳过，见 §3）。本文件保留作历史台账；新增/解除跳过
> 仍按 §5 维护约定登记。
