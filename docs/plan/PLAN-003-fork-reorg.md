...
## 广播互达收敛验证（2026-08-15，机制实证 + 时序敏感）

**机制实证**：分步错峰启动（A→15s→B→15s→C）→ 广播互达 4/6
（A 2、B 0→2、C 0）——入站登记修复有效。
快速错峰（B/C 间隔 5s）→ 1/6——B/C 同时 dial A 竞争。

**结论**：
- P2P 连接/登记机制工作（根因修复：bootstraps @host:port + 入站登记）
- 广播互达收敛**对启动时序敏感**：均匀错峰（节点间隔 10s+）可达 4/6，
  完整 6/6 需更长运行
- 生产部署应均匀错峰启动脚本

**3 节点验证状态**：机制修复完成（4/6 实证），完整收敛待部署脚本
（均匀错峰 + 长运行）。共识/最终性代码不受影响（双节点真机闭环）。
