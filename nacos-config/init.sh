...
echo "[3/4] 发布共享配置..."
publish_config() {
  local dataId="$1" file="$2" ns="${3:-public}"
  if [ ! -f "${file}" ]; then
  ...
  echo "  ${dataId} (ns=${ns}, group=${GROUP}) 发布成功"
}
...
# 4. 发布各微服务私有配置占位（实际配置在 #54/#55 任务中完善）
echo "[4/4] 发布微服务私有配置占位..."
