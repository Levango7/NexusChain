...
public class SigningTccActionTest {
    ...
    public void setUp() {
        ...
        assertNotNull("prikey should not be empty", prikey);
        assertFalse("prikey should not be empty", prikey.isEmpty());
        ...
        assertEquals("address should be valid", 0, WalletUtils.verifyAddress(address));
    }
    ...
    public void testPrepareSignTransfer_success_nonceLocked() {
        ...
        assertTrue("Try should succeed", result);
        ...
        assertEquals("100", ctx.getActionContext("amount", String.class));
    ...
    public void testPrepareSignTransfer_success_rpcGetNonce_whenPoolEmpty() {
        ...
        assertTrue("Try should succeed via RPC getNonce", result);
    ...
    @Test(expected = TccPrepareException.class)
    public void testPrepareSignTransfer_noPlatformKey_throws() {
    ...
    @Test(expected = TccPrepareException.class)
    public void testPrepareSignTransfer_fromPubkeyMismatch_throws() {
        when(platformKeystore.getPrikey()).thenReturn(prikey);
    ...
    @Test(expected = TccPrepareException.class)
    public void testPrepareSignTransfer_rpcGetNonceFailed_throws() {
        when(platformKeystore.getPrikey()).thenReturn(prikey);
        when(platformKeystore.getPubkey()).thenReturn(pubkey);
    ...
    @Test(expected = TccPrepareException.class)
    public void testPrepareSignTransfer_lockConflict_throws() {
        when(platformKeystore.getPrikey()).thenReturn(prikey);
        when(platformKeystore.getPubkey()).thenReturn(pubkey);
    ...
    public void testConfirmSignTransfer_success_signBroadcastRelease() {
        ...
        assertTrue("Confirm should succeed", result);
        ...
        assertNotNull("txHash should be written to context", txHash);
        assertFalse("txHash should not be empty", txHash.isEmpty());
    }
    ...
    public void testConfirmSignTransfer_broadcastFailureStillConfirms() {
        ...
        assertTrue("Confirm should still succeed even if broadcast fails", result);
        verify(noncePool).confirmNonce(eq(address), eq(nonce), anyString());
    }
    ...
    public void testConfirmSignTransfer_missingContext_idempotentSuccess() {
        ...
        BusinessActionContext ctx = newCtx();
        boolean result = tccAction.confirmSignTransfer(ctx);
        assertTrue("Confirm with missing context should return true (idempotent)", result);
    }
    ...
    public void testCancelSignTransfer_noLockRecord_idempotentSuccess() {
        ...
        assertTrue("Cancel should return true even if no lock record (idempotent)", result);
        verify(noncePool).cancelNonce(eq(address), eq(nonce));
    }
    ...
    public void testCancelSignTransfer_missingContext_idempotentSuccess() {
        ...
        BusinessActionContext ctx = newCtx();
        boolean result = tccAction.cancelSignTransfer(ctx);
        assertTrue("Cancel with missing context should return true (Try failed before lock)", result);
    }
