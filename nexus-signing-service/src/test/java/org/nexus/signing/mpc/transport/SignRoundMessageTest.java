...
public class SignRoundMessageTest {
    ...
    public void testBuilderAndGetter5() {
        ...
        assertEquals("r-hex", msg.getRPointHex());
        assertEquals("mta-hex", msg.getMtaShareHex());
        assertEquals("agg-r", msg.getAggregateRHex());
        assertEquals("r-scalar", msg.getRScalarHex());
        assertEquals("zk", msg.getZkProofHex());
        assertEquals("sig", msg.getSigShareHex());
    }
