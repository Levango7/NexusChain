...
class KeyStore {
    ...
    //支付通道开启
    ClientToChannelOpen(fromPubkeyStr, counterpartyPubkeyHashStr, fundingAmount, lockTimeBlocks, prikeyStr, nonce){
        try{
            let isNum = this.numberToString(fundingAmount);
            if(isNum == -1){
                return 5000;
            }
            //版本号
            ...
            let gasPrice = '0000000000000000'.substr(_gasPrice.length) + _gasPrice;
            //通道资金 无符号64位
            let mul = this.mul(fundingAmount);
            if(mul < 0){
                return 5000;
            }
            let _Amount=new Uint64BE(mul,10).toString(16);
            ...
            let signull = this.Bytes2Str(buffersignull);
            //接收者公钥哈希（对手方）
            let toPubkeyHash=counterpartyPubkeyHashStr;
            //构造payload
            let payloadJson = JSON.stringify({lockTimeBlocks: lockTimeBlocks});
            let payloadbyte = Buffer.from(payloadJson);
            let payloadLen = uint32.toHex(payloadbyte.length);
            let payload = this.Bytes2Str(payloadbyte);
            let allPayload = payloadLen + payload;
            let RawTransaction=Buffer.from(version+type+nonece+fromPubkeyHash+gasPrice+Amount+signull+toPubkeyHash+allPayload, 'hex');
    ...
    //支付通道关闭
    ClientToChannelClose(fromPubkeyStr, channelId, finalBalance1, finalBalance2, sig1, sig2, prikeyStr, nonce){
        try{
            //版本号
            let version="01";
            //类型：支付通道关闭
            ...
            let toPubkeyHash=this.Bytes2Str(buffertoPubkeyHash);
            //构造payload
            let payloadJson = JSON.stringify({channelId: channelId, finalBalance1: finalBalance1, finalBalance2: finalBalance2, sig1: sig1, sig2: sig2});
            let payloadbyte = Buffer.from(payloadJson);
            let payloadLen = uint32.toHex(payloadbyte.length);
            let payload = this.Bytes2Str(payloadbyte);
            let allPayload = payloadLen + payload;
    ...
    //批量转账
    ClientToBatchTransfer(fromPubkeyStr, recipients, prikeyStr, nonce){
        try{
            //版本号
            let version="01";
            //类型：批量转账
            ...
            let toPubkeyHash=this.Bytes2Str(buffertoPubkeyHash);
            //构造payload
            let payloadJson = JSON.stringify({recipients: recipients});
            let payloadbyte = Buffer.from(payloadJson);
            let payloadLen = uint32.toHex(payloadbyte.length);
            let payload = this.Bytes2Str(payloadbyte);
            let allPayload = payloadLen + payload;
    ...
    //稳定币铸造
    ClientToMintStableCoin(fromPubkeyStr, collateralAmount, mintAmount, prikeyStr, nonce){
        try{
            let isNum = this.numberToString(collateralAmount);
            if(isNum == -1){
                return 5000;
            }
            //版本号
            ...
            let gasPrice = '0000000000000000'.substr(_gasPrice.length) + _gasPrice;
            //抵押金额 无符号64位
            let mul = this.mul(collateralAmount);
            if(mul < 0){
                return 5000;
            }
            let _Amount=new Uint64BE(mul,10).toString(16);
            ...
            let toPubkeyHash=this.Bytes2Str(buffertoPubkeyHash);
            //构造payload
            let payloadJson = JSON.stringify({mintAmount: mintAmount});
            let payloadbyte = Buffer.from(payloadJson);
            let payloadLen = uint32.toHex(payloadbyte.length);
            let payload = this.Bytes2Str(payloadbyte);
            let allPayload = payloadLen + payload;
    ...
    //稳定币赎回
    ClientToRedeemStableCoin(fromPubkeyStr, redeemAmount, prikeyStr, nonce){
        try{
            let isNum = this.numberToString(redeemAmount);
            if(isNum == -1){
                return 5000;
            }
            //版本号
    ...
    //跨链锁定
    ClientToBridgeLock(fromPubkeyStr, targetChain, recipient, amount, prikeyStr, nonce){
        try{
            let isNum = this.numberToString(amount);
            if(isNum == -1){
                return 5000;
            ...
            let toPubkeyHash=this.Bytes2Str(buffertoPubkeyHash);
            //构造payload
            let payloadJson = JSON.stringify({targetChain: targetChain, recipient: recipient});
            let payloadbyte = Buffer.from(payloadJson);
            let payloadLen = uint32.toHex(payloadbyte.length);
            let payload = this.Bytes2Str(payloadbyte);
            let allPayload = payloadLen + payload;
    ...
    //订阅授权
    ClientToSubscriptionAuth(fromPubkeyStr, merchantPubkeyHash, maxAmount, intervalBlocks, duration, prikeyStr, nonce){
        try{
            let isNum = this.numberToString(maxAmount);
            if(isNum == -1){
                return 5000;
            }
            //版本号
            ...
            let signull = this.Bytes2Str(buffersignull);
            //接收者公钥哈希（商户）
            let toPubkeyHash=merchantPubkeyHash;
            //构造payload
            let payloadJson = JSON.stringify({intervalBlocks: intervalBlocks, duration: duration});
            let payloadbyte = Buffer.from(payloadJson);
            let payloadLen = uint32.toHex(payloadbyte.length);
            let payload = this.Bytes2Str(payloadbyte);
            let allPayload = payloadLen + payload;
    ...
    /**
     * 连接到 ConPay 节点，返回 RPC 客户端实例
     * 用法:
     *   const client = ks.connectToNode('http://localhost:19585');
     *   const balance = await client.sendBalance(pubkeyhash);
     * @param {string} nodeUrl - 节点 RPC 地址，例如 'http://localhost:19585'
     * @returns {ConPayRpcClient} - RPC 客户端实例
     */
    connectToNode(nodeUrl) {
        return new ConPayRpcClient(nodeUrl);
    }
