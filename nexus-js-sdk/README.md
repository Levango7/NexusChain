> **DEPRECATED**: This module has been migrated to nexus-sdk/typescript. Please use the unified SDK.
...
##### sdk所有方法异常返回5000
```
npm i keystore_nexus;
const KeyStore = require('keystore_nexus');
...
#### 构造支付通道开启事务
```
const channelOpen = ks.ClientToChannelOpen(fromPubkey,counterpartyPubkeyHash,fundingAmount,lockTimeBlocks,prikeyStr,nonce);

fromPubkey：发起者公钥
counterpartyPubkeyHash：对手方公钥哈希
fundingAmount：通道资金金额(必须是字符串！)
lockTimeBlocks：锁定时间（区块数）
prikeyStr：私钥
nonce：nonce(通过节点获取)
```
* 返回：
...
#### 构造支付通道关闭事务
```
const channelClose = ks.ClientToChannelClose(fromPubkey,channelId,finalBalance1,finalBalance2,sig1,sig2,prikeyStr,nonce);
...
#### 构造批量转账事务
```
const batchTransfer = ks.ClientToBatchTransfer(fromPubkey,recipients,prikeyStr,nonce);

fromPubkey：发起者公钥
recipients：接收者数组，格式为 [{address: "公钥哈希", amount: "金额"}]
prikeyStr：私钥
nonce：nonce(通过节点获取)
```
* 返回：
...
#### 构造稳定币铸造事务
```
const mintStableCoin = ks.ClientToMintStableCoin(fromPubkey,collateralAmount,mintAmount,prikeyStr,nonce);

fromPubkey：发起者公钥
collateralAmount：抵押金额(必须是字符串！)
mintAmount：铸造金额
prikeyStr：私钥
nonce：nonce(通过节点获取)
```
...
#### 构造稳定币赎回事务
```
const redeemStableCoin = ks.ClientToRedeemStableCoin(fromPubkey,redeemAmount,prikeyStr,nonce);

fromPubkey：发起者公钥
redeemAmount：赎回金额(必须是字符串！)
prikeyStr：私钥
nonce：nonce(通过节点获取)
```
* 返回：
...
#### 构造跨链锁定事务
```
const bridgeLock = ks.ClientToBridgeLock(fromPubkey,targetChain,recipient,amount,prikeyStr,nonce);
...
#### 构造订阅授权事务
```
const subscriptionAuth = ks.ClientToSubscriptionAuth(fromPubkey,merchantPubkeyHash,maxAmount,intervalBlocks,duration,prikeyStr,nonce);

fromPubkey：发起者公钥
merchantPubkeyHash：商户公钥哈希
maxAmount：最大授权金额(必须是字符串！)
intervalBlocks：扣款间隔（区块数）
duration：授权持续时间（区块数）
prikeyStr：私钥
nonce：nonce(通过节点获取)
```
